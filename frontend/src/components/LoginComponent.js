    ...
    const handleLogin = () => {
        ...
        axios.post('http://127.0.0.1:8000/login/', formData, {
            ...
            .then(response => {
                ...
                // Store the token securely, consider using HttpOnly cookies or other methods.
            ...
            .catch(error => {
                ...
                if (error.response && error.response.data && error.response.data.detail) {
                    if (typeof error.response.data.detail === 'string') {
                        ...
                        errMsg = error.response.data.detail.msg;
                    }
                }
    ...
    return (
        <div className="h-screen w-full flex items-center justify-center bg-gray-200"> {/* This surrounds the card and centers it */}
            <div className="flex flex-col items-center justify-center rounded-lg shadow-sm hover:shadow-2xl transition-shadow duration-500 bg-gray-50 py-4 px-4 sm:px-6 lg:px-8 w-96"> {/* w-96 sets a fixed width, adjust as needed */}
                {isLoggedIn ? (
                    <p className="mb-4 text-center font-mono text-green-300 bg-green-700 rounded border animate-bounce ">{message}<a href="/" className="text-blue-700 hover:text-blue-400">{" Click here."}</a></p>
                ) : (
